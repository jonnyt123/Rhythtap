...
export function chartV4RolloutTransform():Plugin{return{name:'rhythtap-chart-v4-rollout-transform',enforce:'pre',transform(source,id){const path=id.replaceAll('\\','/');let code=source;
 if(path.endsWith('/src/player-account.tsx')){code=replaceRequired(code,'solo chart version',"body:JSON.stringify({songId:input.songId,difficulty:input.difficulty,events:input.events})","body:JSON.stringify({songId:input.songId,difficulty:input.difficulty,chartVersion:4,events:input.events})");return{code,map:null}}

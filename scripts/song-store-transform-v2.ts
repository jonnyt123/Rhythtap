...
const main=(source:string)=>{
 ...
 code=required(code,'guest coins',"localStorage.setItem('rhythtap-profile',JSON.stringify({xp}));setResult(","localStorage.setItem('rhythtap-profile',JSON.stringify({xp}));setLocalSongEconomy(current=>awardLocalCoins(current,xpEarned));setResult(");

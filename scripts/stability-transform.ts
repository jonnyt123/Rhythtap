...
const patchMain=(source:string)=>{
 ...
 code=replaceRequired(code,'solo judgement buffer',"backgroundPaused=useRef(false),failedRef=useRef(false),energyRef=useRef(100);","backgroundPaused=useRef(false),failedRef=useRef(false),energyRef=useRef(100),soloEvents=useRef<GameplayJudgementEvent[]>([]);");
 ...
 code=replaceRequired(code,'natural media completion',
  ...
  "const mediaFinished=transport.current.ended()&&t>=notes.at(-1)!.time+TIMING.good;if((t>end||mediaFinished)&&!failedRef.current&&energyRef.current>0){transport.current.stop();const c=countsRef.current,total=Object.values(c).reduce((a,b)=>a+b,0),weighted=c.PERFECT+c.GREAT*.8+c.GOOD*.5,finalResult={score:scoreRef.current,accuracy:total?weighted/total*100:0,maxCombo:maxRef.current,counts:c,events:soloEvents.current.slice()};if(multiplayerSession.enabled)multiplayerSession.publishFinal({score:finalResult.score,combo:finalResult.maxCombo,accuracy:finalResult.accuracy});finish(finalResult);return}");
 code=replaceRequired(code,'validated high score persistence',
  " useEffect(()=>{const key=`ntr-high-${song.id}-${difficulty}${difficulty==='HARD'&&!song.id.startsWith('tap-')?'-v5':''}`;localStorage.setItem(key,String(Math.max(result.score,Number(localStorage.getItem(key)||0))))},[]);",

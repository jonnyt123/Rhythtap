...
export function tourSocialRankedTransform():Plugin{
 return {name:'rhythtap-tour-social-ranked-transform',enforce:'pre',transform(source,id){
  ...
  const featureButtons='<button className="metal-menu-button metal-menu-social" onClick={onSocial}><span className="metal-menu-icon"><User/></span><span className="metal-menu-copy"><small>SOCIAL</small><strong>FRIENDS + INVITES</strong><em>Friends online · direct battles</em></span><ChevronRight/></button>\n   <button className="metal-menu-button metal-menu-ranked" onClick={onRanked}><span className="metal-menu-icon"><Award/></span><span className="metal-menu-copy"><small>COMPETITIVE</small><strong>RANKED</strong><em>Battle rating · leaderboards</em></span><ChevronRight/></button>\n   '+onlineButton;

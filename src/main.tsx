...
function Game({song,speed,offset,quit,finish}:{song:Song,speed:number,offset:number,quit:()=>void,finish:(r:any)=>void}){
 ...
 const begin=async()=>{judged.current.clear();await transport.current.start(song);started.current=true;setReady(true)};
 useEffect(()=>()=>{cancelAnimationFrame(raf.current);transport.current.stop()},[]);
 useEffect(()=>{if(!ready||paused)return;const tick=()=>{const t=transport.current.now()+offset;setNow(t);for(const n of song.notes){if(!judged.current.has(n.id)&&t-n.time>180){judged.current.add(n.id);applyJudge('MISS')}}if(t>end){transport.current.stop();const c=countsRef.current,total=Object.values(c).reduce((a,b)=>a+b,0),weighted=c.PERFECT+c.GREAT*.8+c.GOOD*.5;finish({score:scoreRef.current,accuracy:total?weighted/total*100:0,maxCombo:maxRef.current,counts:c});return}raf.current=requestAnimationFrame(tick)};raf.current=requestAnimationFrame(tick);return()=>cancelAnimationFrame(raf.current)},[ready,paused]);
 const applyJudge=(j:Judge)=>{setJudge(j);window.setTimeout(()=>setJudge(null),180);setCounts(c=>({...c,[j]:c[j]+1}));if(j==='MISS'){setCombo(0);setEnergy(e=>Math.max(0,e-8));return}const add={PERFECT:1000,GREAT:700,GOOD:350,MISS:0}[j];setScore(s=>s+add+combo*8);setCombo(c=>{const n=c+1;setMaxCombo(m=>Math.max(m,n));return n});setPulse(p=>Math.min(100,p+(j==='PERFECT'?5:2)));setEnergy(e=>Math.min(100,e+1))};
 const hit=(lane:number)=>{if(!ready||paused)return;pressed.current.add(lane);let best:Note|undefined,dist=Infinity;for(const n of song.notes){const d=Math.abs(now-n.time);if(n.lane===lane&&!judged.current.has(n.id)&&d<dist){best=n;dist=d}}if(!best||dist>180)return;judged.current.add(best.id);applyJudge(dist<=45?'PERFECT':dist<=90?'GREAT':'GOOD')};
 useEffect(()=>{const down=(e:KeyboardEvent)=>{const i=['KeyD','KeyF','KeyJ','KeyK'].indexOf(e.code);if(i>=0){e.preventDefault();hit(i)}if(e.code==='Escape')togglePause()};const up=(e:KeyboardEvent)=>pressed.current.delete(['KeyD','KeyF','KeyJ','KeyK'].indexOf(e.code));addEventListener('keydown',down);addEventListener('keyup',up);return()=>{removeEventListener('keydown',down);removeEventListener('keyup',up)}},[now,ready,paused]);
 const togglePause=async()=>{if(!ready)return;if(paused){await transport.current.resume();setPaused(false)}else{transport.current.pause();setPaused(true)}};
 ...
 return <section className="game screen"><div className="gamehud"><button className="icon" onClick={quit}><ArrowLeft/></button><div className="score"><small>SCORE</small><strong>{score.toString().padStart(7,'0')}</strong></div><button className="icon" onClick={togglePause}>{paused?<Play/>:<Pause/>}</button></div>
  ...
  <div className="arena">{LANES.map((k,l)=><div className="lane" key={k} style={{'--lane':COLORS[l]} as React.CSSProperties}>{song.notes.filter(n=>n.lane===l&&!judged.current.has(n.id)).map(n=>{const y=(now-(n.time-travel))/travel*100;return y>-12&&y<115?<div key={n.id} className={'note '+(n.duration?'hold':'')} style={{top:y+'%',height:n.duration?Math.max(28,n.duration/travel*70)+'px':undefined}}/>:null})}<button className="pad" onPointerDown={e=>{e.currentTarget.setPointerCapture(e.pointerId);hit(l)}}><span>{k}</span></button></div>)}</div>

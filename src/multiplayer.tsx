...
export type MultiplayerSong={id:string,title:string,artist:string,unlockLevel:number,charts:Record<MultiplayerDifficulty,unknown[]>};
...
export type MultiplayerSession={enabled:boolean,launch:MultiplayerLaunch|null,connected:boolean,opponent:MultiplayerProgress|null,publishProgress:(progress:Omit<MultiplayerProgress,'playerId'|'name'>)=>void,publishFinal:(progress:Omit<MultiplayerProgress,'playerId'|'name'|'finished'>)=>void};
...
export function MultiplayerLobby({songs,profileLevel,initialDifficulty,onBack,onLaunch}:{songs:MultiplayerSong[],profileLevel:number,initialDifficulty:MultiplayerDifficulty,onBack:()=>void,onLaunch:(launch:MultiplayerLaunch)=>void}){
 ...
 const presencePayload=useCallback(()=>({playerId:idRef.current,name:(name.trim()||'PLAYER').slice(0,18),isHost:hostRef.current,songId,difficulty,joinedAt:Date.now()}),[name,songId,difficulty]);
 useEffect(()=>{localStorage.setItem(DISPLAY_NAME_KEY,(name.trim()||'PLAYER').slice(0,18));const channel=channelRef.current;if(channel&&status==='connected')void channel.track(presencePayload())},[name,songId,difficulty,status,presencePayload]);
 ...
 const connect=async(nextCode:string,host:boolean)=>{
  ...
  await cleanup();setStatus('connecting');setMessage('CONNECTING TO REALTIME…');setPlayers([]);hostRef.current=host;setIsHost(host);setRoomCode(normalized);setCode(normalized);
  try{
 ...
 const chooseSong=(next:string)=>{if(!isHost)return;setSongId(next);broadcastSettings(next,difficulty)};
 const chooseDifficulty=(next:MultiplayerDifficulty)=>{if(!isHost)return;setDifficulty(next);broadcastSettings(songId,next)};
 const startMatch=()=>{const channel=channelRef.current;if(!channel||!isHost||players.length<2||!selectedSong)return;const startAt=Date.now()+4000;void channel.send({type:'broadcast',event:'start',payload:{songId:selectedSong.id,difficulty,startAt}})};
 ...
 if(roomCode)return <section className="multiplayer screen">
  ...
  <div className="mp-room-footer"><div><Radio/><span><strong>{message}</strong><small>{opponentCount?`${opponentCount} opponent connected`:'Waiting for a second player'}</small></span></div>{isHost?<button className="primary" disabled={status!=='connected'||players.length<2||players.length>2} onClick={startMatch}><Play fill="currentColor"/> START BATTLE</button>:<div className="mp-wait"><LoaderCircle/> HOST STARTS THE MATCH</div>}</div>
 </section>;
...
export function useMultiplayerSession(launch:MultiplayerLaunch|null):MultiplayerSession{
 ...
 return useMemo(()=>({enabled:Boolean(launch),launch,connected,opponent,publishProgress,publishFinal}),[launch,connected,opponent,publishProgress,publishFinal]);
}

...
const padTouchStyle={userSelect:'none',WebkitUserSelect:'none',touchAction:'none',WebkitTouchCallout:'none'} as React.CSSProperties;
...
function Intro(){return <div className="tutorial-card intro-card"><div className="tutorial-logo"><Zap fill="currentColor"/></div><small>WELCOME TO RHYTHMTAP</small><h1>FOLLOW THE NOTES.<br/><i>HIT THE BEAT.</i></h1><p>Notes fall down one of three lanes. Tap the matching lane when the note reaches its glowing circular receptor.</p><div className="three-rules"><span><b>1</b> WATCH</span><span><b>2</b> MATCH</span><span><b>3</b> TAP</span></div></div>}
...
function PlayDemo({lane,mode,hits,holding,feedback,onTap,onStart,onEnd}:{lane:Lane,mode:DemoMode,hits:number,holding:boolean,feedback:string,onTap:(lane:Lane)=>void,onStart:(lane:Lane)=>void,onEnd:(lane:Lane)=>void}){
 ...
 const pointerDown=(event:React.PointerEvent<HTMLButtonElement>,index:Lane)=>{event.preventDefault();if(mode==='tap')onTap(index);else onStart(index)};
 const pointerUp=(event:React.PointerEvent<HTMLButtonElement>,index:Lane)=>{event.preventDefault();if(mode==='hold')onEnd(index)};
 const keyboardClick=(event:React.MouseEvent<HTMLButtonElement>,index:Lane)=>{if(mode==='tap'&&event.detail===0)onTap(index)};
 return <div className="tutorial-play-card"><div className="tutorial-copy"><small>{mode==='tap'?'TAP NOTES':'HOLD NOTES'}</small><h1>{mode==='tap'?'TAP AS IT REACHES THE CIRCLE':'PRESS, HOLD, THEN RELEASE'}</h1><p>{mode==='tap'?'Watch which lane the note is falling through, then tap when it overlaps that lane’s glowing receptor.':'Long notes have a tail. Start the hold at the receptor and keep your finger down until the tail finishes.'}</p></div><div className="demo-stage"><div className="demo-lanes">{([0,1,2] as Lane[]).map(index=><div className="demo-lane" key={index}><span>{laneLabels[index]}</span>{index===lane&&<div key={noteKey} className={'demo-note '+(mode==='hold'?'hold':'')}><i/></div>}</div>)}</div><div className="demo-receptors" aria-hidden="true">{([0,1,2] as Lane[]).map(index=><i key={index}/>)}</div><div className="demo-pads">{([0,1,2] as Lane[]).map(index=><button key={index} draggable={false} style={padTouchStyle} className={holding&&index===lane?'pressed':''} onContextMenu={event=>event.preventDefault()} onPointerDown={event=>pointerDown(event,index)} onPointerUp={event=>pointerUp(event,index)} onPointerCancel={event=>pointerUp(event,index)} onClick={event=>keyboardClick(event,index)}><span/></button>)}</div></div><div className="tutorial-feedback"><strong>{feedback||'WATCH THE NOTE FALL'}</strong><span>{hits} / {mode==='tap'?3:2}</span></div></div>;
}

function JudgmentStep(){return <div className="tutorial-card"><small>YOUR TIMING MATTERS</small><h1>THE CLOSER TO THE RECEPTOR CENTER,<br/><i>THE BETTER THE HIT.</i></h1><div className="judgment-demo"><span><b>PERFECT</b><small>Right on the beat</small></span><span><b>GREAT</b><small>Very close</small></span><span><b>GOOD</b><small>Still counts</small></span><span><b>MISS</b><small>Too early or late</small></span></div><p>Build a combo by avoiding misses. Accuracy, combo and score all improve when your timing gets tighter.</p></div>}

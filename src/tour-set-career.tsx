...
export async function recordTourSetResult(userId:string|null,run:TourSetRun|null,score:number,accuracy:number,difficulty:TourDifficulty){
 ...
 try{
  ...
  await client.from('player_tour_progress').upsert({user_id:userId,gig_id:id,stars:Math.max(stars,Number(cloud.stars)||0),best_score:Math.max(score,Number(cloud.best_score)||0),best_accuracy:Math.max(accuracy,Number(cloud.best_accuracy)||0),best_difficulty:difficultyRank(difficulty)>=difficultyRank((cloud.best_difficulty||'EASY') as TourDifficulty)?difficulty:(cloud.best_difficulty||'EASY'),completed_at:stars>0?(cloud.completed_at||new Date().toISOString()):cloud.completed_at,updated_at:new Date().toISOString()});
...
export function TourSetScreen({songs,userId,back,onPlay}:{songs:TourSong[],profileLevel:number,userId:string|null,back:()=>void,onPlay:(songId:string,difficulty:TourDifficulty,run:TourSetRun)=>void}){
 const[progress,setProgress]=useState<TourProgress[]>(()=>readLocal(userId)),[loading,setLoading]=useState(Boolean(userId));
 useEffect(()=>{let mounted=true;const local=readLocal(userId);setProgress(local);setLoading(Boolean(userId));(async()=>{if(!userId){setLoading(false);return}try{const client=await getClient(),{data}=await client.from('player_tour_progress').select('*').eq('user_id',userId).like('gig_id','tourset:%');if(!mounted)return;const cloud=(data||[]).map(cloudRow).filter(Boolean) as TourProgress[],merged=mergeProgress(local,cloud);setProgress(merged);saveLocal(userId,merged)}catch(error){console.warn('[tour-set] cloud load failed',error)}finally{if(mounted)setLoading(false)}})();return()=>{mounted=false}},[userId]);
 ...
 const setComplete=(set:TourSet)=>set.songs.every((_,slot)=>starsAt(set.id,slot)>0);
 ...
 const firstIncomplete=(set:TourSet)=>{const slot=set.songs.findIndex((_,index)=>starsAt(set.id,index)===0);return slot<0?0:slot};
 ...
 const startSlot=(set:TourSet,index:number,slot:number)=>{const spec=set.songs[slot],song=songMap.get(spec.songId);if(!song||!isUnlocked(set,index))return;onPlay(song.id,spec.difficulty,{gigId:set.id,slot})};
 return <section className="career-tour screen">
  ...
  <div className="career-wrap">
   <div className="career-hero"><div><small>CAREER PROGRESS</small><strong>{totalStars} <span>/ {MAX_STARS} STARS</span></strong><p>Every gig is a three-song set. Clear each song with at least 70% accuracy to advance. Replay sets to raise your star total.</p><div className="career-star-key"><span><Star/>70% = 1</span><span><Star/>85% = 2</span><span><Star/>95% = 3</span></div></div><Trophy/></div>
   <div className="career-path">{tourSets.map((set,index)=>{const unlocked=isUnlocked(set,index),complete=setComplete(set),setStars=set.songs.reduce((sum,_,slot)=>sum+starsAt(set.id,slot),0),nextSlot=firstIncomplete(set);const gate=set.headliner&&!unlocked&&setComplete(tourSets[index-1])?`${HEADLINER_STAR_GATE-totalStars} MORE STARS TO HEADLINE`:`CLEAR ${index>0?tourSets[index-1].venue:'THIS SET'} TO ADVANCE`;return <article key={set.id} className={`career-gig${set.headliner?' headliner':''}${unlocked?'':' locked'}${complete?' complete':''}`}>
    ...
    <div className="career-setlist">{set.songs.map((spec,slot)=>{const song=songMap.get(spec.songId),stars=starsAt(set.id,slot),previousCleared=slot===0||starsAt(set.id,slot-1)>0,available=unlocked&&(complete||previousCleared)&&(slot<=nextSlot||stars>0),cleared=stars>0;return <div className={`career-song${cleared?' cleared':''}${available?'':' unavailable'}`} key={`${set.id}-${slot}`}><span className="career-song-index">{cleared?<Check/>:available?slot+1:<Lock/>}</span><div className="career-song-copy"><small>SONG {slot+1} · {spec.difficulty}</small><strong>{song?.title||spec.songId}</strong><span>{song?.artist||'TRACK UNAVAILABLE'}</span></div><div className="career-song-stars">{[1,2,3].map(value=><Star key={value} fill={stars>=value?'currentColor':'none'}/>)}</div><button disabled={!available||!song||loading} onClick={()=>startSlot(set,index,slot)} aria-label={`Play ${song?.title||spec.songId}`}><Play/></button></div>})}</div>
    <div className="career-gig-footer"><div>{complete?<><Check/><span><small>SET COMPLETE</small><strong>{set.reward}</strong></span></>:unlocked?<><Music2/><span><small>{nextSlot}/3 SONGS CLEARED</small><strong>FINISH ALL THREE TO ADVANCE</strong></span></>:<><Lock/><span><small>LOCKED</small><strong>{gate}</strong></span></>}</div><button className="career-set-button" disabled={!unlocked||loading} onClick={()=>startSlot(set,index,complete?0:nextSlot)}>{set.headliner?<Crown/>:<Play/>}{complete?'REPLAY SET':nextSlot===0?(set.headliner?'TAKE THE STAGE':'START SET'):`CONTINUE · SONG ${nextSlot+1}`}</button></div>
   </article>})}</div>

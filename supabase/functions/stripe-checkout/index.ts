...
Deno.serve(async req=>{
 ...
 try{
  const user=await authenticatedUser(req),body=await req.json().catch(()=>({})),interval=body?.interval==='monthly'?'monthly':'annual',environment='live';
  ...
  return json({url:url.toString(),paymentLink:true,interval,environment});
 }catch(error){

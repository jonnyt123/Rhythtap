...
const LIVE_PORTAL_LOGIN='https://billing.stripe.com/p/login/14A7sL2BA6CX1dD1Fe7ss00';
...
Deno.serve(async req=>{
 ...
 try{
  const userId=await authenticatedUserId(req),environment=billingEnvironment();
  if(environment!=='live')throw new Error('Sandbox Customer Portal login is not configured');
  const admin=adminClient(),{data:billing,error}=await admin.from('player_billing_entitlements').select('stripe_customer_id').eq('user_id',userId).eq('environment',environment).maybeSingle();
  if(error)throw error;
  if(!billing?.stripe_customer_id)return json({error:'No Stripe customer is linked to this RhythmTap ID yet.'},404);
  return json({url:LIVE_PORTAL_LOGIN,noCodePortal:true});
 }catch(error){

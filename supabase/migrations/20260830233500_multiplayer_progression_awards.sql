...
begin
  ...
  insert into public.player_progress_events (
    ...
    validated.max_combo,
    coalesce((select count(*) from jsonb_array_elements('[]'::jsonb)), 0),
    award, bonus, p_match_id
  );

  -- Perfect-hit count is reconstructed from the immutable result's event data only
  -- inside the validator today, while multiplayer_results stores aggregate score data.
  -- Do not fabricate perfect hits here; battle XP and score progression remain authoritative.

...
const assertHardDensity=(name:string,notes:{time:number}[])=>{for(let i=0;i<notes.length;i++){const start=notes[i].time;let count=0;for(let j=i;j<notes.length&&notes[j].time<start+1000;j++)count++;if(count>6)throw new Error(`${name}: ${count} notes inside one second exceeds Hard performance cap`)}const events=[...new Set(notes.map(note=>note.time))];for(let i=1;i<events.length;i++)if(events[i]-events[i-1]<125)throw new Error(`${name}: event spacing ${events[i]-events[i-1]}ms is below 125ms`)};

Deno.test('V5 preserves Easy and Normal exactly',()=>{for(const difficulty of ['EASY','NORMAL'] as const){for(const [,bpm,offset,duration,seed] of beatSongs){const v4=makeV4Beat(bpm,offset,duration,difficulty,seed),v5=makeV5Beat(bpm,offset,duration,difficulty,seed);if(JSON.stringify(v4)!==JSON.stringify(v5))throw new Error(`${difficulty} changed in V5`)}}});
Deno.test('V5 Hard reduces peak density and remains authoritative for every beat-grid song',()=>{for(const [name,bpm,offset,duration,seed] of beatSongs){const v4=makeV4Beat(bpm,offset,duration,'HARD',seed),v5=makeV5Beat(bpm,offset,duration,'HARD',seed),server=buildV5(name,'HARD').notes;if(v5.length>=v4.length)throw new Error(`${name}: V5 Hard did not reduce note count`);if(JSON.stringify(v5)!==JSON.stringify(server))throw new Error(`${name}: client/server V5 mismatch`);assertHardDensity(name,v5)}});

...
Deno.test('v5 request versioning is consolidated into production source',async()=>{
 const stability=await Deno.readTextFile('scripts/stability-transform.ts');
 const common=await Deno.readTextFile('src/multiplayer-common.ts');
 const session=await Deno.readTextFile('src/multiplayer-session.ts');
 const vite=await Deno.readTextFile('vite.config.ts');
 if(!stability.includes("body:JSON.stringify({songId:input.songId,difficulty:input.difficulty,chartVersion:5,events:input.events})"))throw new Error('solo validation request must send chartVersion 5');
 if(!common.includes("functionRequest({action:'register',chartVersion:5,...input})"))throw new Error('match registration must send chartVersion 5');
 if(!session.includes("functionRequest({action:'finalize',chartVersion:5,"))throw new Error('match finalize must send chartVersion 5');
 if(vite.includes('chartV4RolloutTransform')||vite.includes('chart-v4-rollout-transform'))throw new Error('chart version rollout must not depend on a Vite transform');
});
